        ...
        <div class="col-md-12">
            @can('agent_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.agents.create') }}">
                            ...
                            <tbody>
                                @foreach($agents as $key => $agent)
                                    <tr data-entry-id="{{ $agent->id }}">
                                        ...
                                        <td>
                                            @can('agent_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.agents.show', $agent->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('agent_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.agents.edit', $agent->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('agent_delete')
                                                <form action="{{ route('frontend.agents.destroy', $agent->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('agent_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.agents.massDestroy') }}",
