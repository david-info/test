...
@can('marchant_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.marchants.create') }}">
                ...
                <tbody>
                    @foreach($marchants as $key => $marchant)
                        <tr data-entry-id="{{ $marchant->id }}">
                            ...
                            <td>
                                @can('marchant_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.marchants.show', $marchant->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('marchant_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.marchants.edit', $marchant->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('marchant_delete')
                                    <form action="{{ route('admin.marchants.destroy', $marchant->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('marchant_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.marchants.massDestroy') }}",
